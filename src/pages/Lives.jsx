...
const Lives = () => {
  ...
  const getShareUrl = (withTime = true) => {
    const baseUrl = window.location.origin + currentPathLink + "/" + vidId + "/" + name;
      ...
      {isSharePopupOpen && (
        <div className="fixed inset-0 w-full h-[92dvh] m md:h-screen flex items-end md:items-center justify-center z-[999] animate-fade-in rounded">
          ...
          <div className="relative bg-gray-900 rounded-xl p-6  mx-4 shadow-2xl animate-scale-in border border-purple-500/10">
            ...
            <div className="flex items-center gap-3 mb-6 p-3 bg-purple-500/5 rounded-lg border border-purple-500/20">
              <input
                type="checkbox"
                id="includeTime"
                checked={includeTime}
                onChange={(e) => setIncludeTime(e.target.checked)}
                className="w-4 h-4 rounded bg-gray-700 border-purple-500/30 text-purple-500 focus:ring-purple-500/30"
              />
              <label htmlFor="includeTime" className="text-sm text-gray-300 select-none">
              ...
              {[
                {
                  ...
                  color: 'blue',
                  onClick: () => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(document.title)}&url=${encodeURIComponent(getShareUrl(includeTime))}`, "_blank")
                },
                {
                  name: 'LinkedIn',
                  ...
                  color: 'sky',
                  onClick: () => window.open(`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(getShareUrl(includeTime))}`, "_blank")
                },
                {
                  name: 'WhatsApp',
                  ...
                  color: 'green',
                  onClick: () => window.open(`https://wa.me/?text=${encodeURIComponent(`${document.title} ${getShareUrl(includeTime)}`)}`, "_blank")
                },
                {
                  name: t("common.shareGeneric"),
                  icon: <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8" /><polyline points="16 6 12 2 8 6" /><line x1="12" y1="2" x2="12" y2="15" /></svg>,
                  color: 'purple',
                  onClick: () => navigator.share({ title: document.title, url: getShareUrl(includeTime) }).catch(console.error)
                }
              ].map(({ name, icon, color, onClick }) => (
                <button
                  key={name}
                  onClick={onClick}
                  className={`flex items-center justify-center gap-2 p-3 text-${color}-400 border border-${color}-500/30 
                    rounded-lg transition-all duration-300 hover:bg-${color}-500/10 hover:scale-105 active:scale-95 
                    hover:border-${color}-500/50 group`}
                >
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 p-4 text-purple-300 
                bg-purple-500/5 border border-purple-500/30 rounded-lg transition-all duration-300 
                hover:bg-purple-500/10 hover:scale-[1.02] active:scale-[0.98] hover:border-purple-500/50"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71" />
                <path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71" />
              </svg>
              <span className="font-medium">{t("common.copyUrl")}</span>
            </button>
          </div>
        </div>
      )}
